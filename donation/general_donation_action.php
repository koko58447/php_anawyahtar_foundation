...
if($action == 'show'){  
    ...
    $search = $_POST['search'];
    if($search == ''){         
        $sql="select * from tblmain_donation where Status='general' 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){         
            $sql_total="select AID from tblmain_donation where Status='general' 
            order by AID desc";
        }else{ 
            $sql_total="select AID from tblmain_donation where Status='general' 
            and (Donater like '%$search%' or Phno like '%$search%' or 
...
if($action == 'save'){       
    ...
    values ('{$dname}','{$address}','{$phno}','{$done}','{$no}','{$amount}','{$date}','{$rmk}','general')";
    
    if(mysqli_query($con,$sql)){
        save_log($_SESSION['username']."သည် နဝကမအလှူရှင်အား အသစ်သွင်းသွားသည်။ ");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'edit'){ 
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION['username']."သည် နဝကမအလှူရှင်အား ပြင်ဆင်သွားသည်။ ");          
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'delete'){
    ...
    if(mysqli_query($con,$sql)){
        save_log($_SESSION['username']."သည် နဝကမအလှူရှင်အား ဖျက်သွားသည်။ ");
        echo 1;
    }
    else{
        echo 0;
...
if($action=='excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "နဝကမအလှူရှင် Report_".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '<head><meta charset="utf-8" /></head>
        <table >  
            <tr>
            <td colspan="9" align="center"><h3>အနော်ရထာတက္ကသိုလ် နဝကမအလှူရှင်းများ စာရင်း</h3></td>
            </tr>
