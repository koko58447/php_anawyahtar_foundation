...
if($action == 'show'){  
    ...
    $search = $_POST['search'];
    if($search == ''){         
        $sql="select i.*,d.Name as dname,s.Student_Name,t.Name as tname  
        from tblinherit_donation i,tbldonater d,tblstudent s,tblteacher t 
        where i.DonaterID=d.AID and i.StudentID=s.AID and i.TeacherID=t.AID  
        order by i.AID desc limit $offset,$limit_per_page";
    }else{ 
        $sql="select i.*,d.Name as dname,s.Student_Name,t.Name as tname  
        from tblinherit_donation i,tbldonater d,tblstudent s,tblteacher t 
        where i.DonaterID=d.AID and i.StudentID=s.AID and i.TeacherID=t.AID  
        and (d.Name like '%$search%' or s.Student_Name like '%$search%' or 
        t.Name like '%$search%') order by i.AID desc limit $offset,$limit_per_page";  
    }
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        if($search == ''){         
            ...
            where i.DonaterID=d.AID and i.StudentID=s.AID and i.TeacherID=t.AID  
            and (d.Name like '%$search%' or s.Student_Name like '%$search%' or 
            t.Name like '%$search%') order by i.AID desc";  
        }
...
if($action == 'save'){       
    ...
    $sql="insert into tblinherit_donation (DonaterID,StudentID,TeacherID,Done,No,Amount,Date,Rmk) 
    values ('{$dname}','{$sname}','{$tname}','{$done}','{$no}','{$amount}','{$date}','{$rmk}')";
    
    if(mysqli_query($con,$sql)){
        save_log($_SESSION['username']."သည် အမွေခံအလှူအား အသစ်သွင်းသွားသည်။ ");
        echo 1;
    }else{
        echo 0;
    }
...
if($action=='prepare'){
    ...
    $sql="select i.*,d.Name as dname,s.Student_Name,t.Name as tname  
    from tblinherit_donation i,tbldonater d,tblstudent s,tblteacher t 
    where i.DonaterID=d.AID and i.StudentID=s.AID and i.TeacherID=t.AID and i.AID=$aid";
            ...
            <input type='hidden' name='aid' value='{$row["AID"]}'>
                <div class='modal-body row'>                    
                    <div class='col-md-6 form-group'>
                        ...
                        <select class='form-control text-primary' name='edname'>
                            <option value='{$row["DonaterID"]}'>{$row["dname"]}</option>";
                            $out.= load_donater();
                        $out.="</select>
                    </div>
                    <div class='col-md-6 form-group'>
                        <label for=''>ရှင်သာမဏေ</label>
                        <select class='form-control text-primary' name='esname'>
                            <option value='{$row["StudentID"]}'>{$row["Student_Name"]}</option>";
                            $out.= load_student();
                        $out.="</select>
                    </div>
                    <div class=' col-md-6 form-group'>
                        <label for=''>ဆရာ</label>
                        <select class='form-control text-primary' name='etname'>
                            <option value='{$row["TeacherID"]}'>{$row["tname"]}</option>";
                            $out.= load_teacher();
                        $out.="</select>
                    </div>
                    <div class='col-md-6 form-group'>
...
if($action == 'edit'){ 
    ...
    $sql="update tblinherit_donation set DonaterID='{$dname}',StudentID='{$sname}',TeacherID='{$tname}',
    Done='{$done}',No='{$no}',Amount='{$amount}',Date='{$date}',Rmk='{$rmk}' where AID={$aid}";
    
    if(mysqli_query($con,$sql)){  
        save_log($_SESSION['username']."သည် အမွေခံအလှူအား ပြင်ဆင်သွားသည်။ ");        
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'delete'){
    $aid = $_POST["aid"];
    $sql = "delete from tblinherit_donation where AID=$aid";     
    if(mysqli_query($con,$sql)){
        save_log($_SESSION['username']."သည် အမွေခံအလှူအား ဖျက်သွားသည်။ ");
        echo 1;
    }
    else{
        echo 0;
...
if($action=='excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "အမွေခံအလှူ Report_".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '<head><meta charset="utf-8" /></head>
        <table >  
            <tr>
                <td colspan="9" align="center"><h3>အနော်ရထာတက္ကသိုလ် အမွေခံအလှူရှင်များ စာရင်း</h3></td>
            </tr>
            <tr><td colspan="9"><td></tr>
            <tr>                
                <th style="border: 1px solid ;">စဉ်</th>
                <th style="border: 1px solid ;">အလှူရှင်အမည်</th>
                <th style="border: 1px solid ;">ရှင်သာမနေ</th>
                <th style="border: 1px solid ;">ဆရာ</th>
                <th style="border: 1px solid ;">အကျိုးဆောင်</th>
                <th style="border: 1px solid ;">မှတ်တမ်းဝင်အမှတ်</th>
                <th style="border: 1px solid ;">အလှူငွေ</th>
                <th style="border: 1px solid ;">ရက်စွဲ</th>
        ...
        while($row = mysqli_fetch_array($result))
        {
            ...
            $out .= '
                <tr>  
                    ...
                    <td style="border: 1px solid ;">'.$row["tname"].'</td>  
        ...
        <table >  
            ...
            <th style="border: 1px solid ;">စဉ်</th>
            <th style="border: 1px solid ;">အလှူရှင်အမည်</th>
            <th style="border: 1px solid ;">ရှင်သာမဏေ</th>
            <th style="border: 1px solid ;">ဆရာ</th>
            <th style="border: 1px solid ;">အကျိုးဆောင်</th>
            <th style="border: 1px solid ;">မှတ်တမ်းဝင်အမှတ်</th>
