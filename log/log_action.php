...
if($_POST["action"] == 'show'){  
    ...
    if($_POST['from']!='' && $_POST['to']!=''){
        $chkdate=" and Date(l.Date)>='{$_POST['from']}' and Date(l.Date)<='{$_POST['to']}' ";
        $chkdate1=" where Date(l.Date)>='{$_POST['from']}' and Date(l.Date)<='{$_POST['to']}' ";
        $chkdate2=" and Date(l.Date)>='{$_POST['from']}' and Date(l.Date)<='{$_POST['to']}' ";
    }                                               
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        $sql_total="";
        if($search == ''){
            $sql_total="select * from tbllog l ".$chkdate1;
        }else{
            $sql_total="select l.*,u.UserName from tbllog l,tbluser u where l.UserID=u.AID ".$chkdate2. "
             and (l.Description like '%$search%' or u.UserName like '%$search%')";
        }
